...
function RecentlyAddedCard(props) {
  ...
  const currentLibraryDefaultIcon =
    props.data.Type === "Movie"
      ...
      : MixedIcon;
  const imageIds = [
    props.data.Type === "Episode" ? props.data.SeriesId : null,
    props.data.Type === "Episode" ? props.data.EpisodeId : null,
    props.data.Id,
  ].filter(Boolean);
  const activeImageId = imageIds[imageIndex];
  const showFallback = !activeImageId || imageIndex >= imageIds.length;
