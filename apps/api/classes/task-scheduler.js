...
class TaskScheduler {
  constructor() {
    ...
    this.defaultIntervals = {
      ...
      JellyfinPlaybackReportingPluginSync: {
        Interval: 1440,
        ...TaskList.JellyfinPlaybackReportingPluginSync,
      },
      RefreshDashboardStats: {
        Interval: 720,
        ...TaskList.RefreshDashboardStats,
      },
      ClearStaleTasks: {
        Interval: 10080,
        ...TaskList.ClearStaleTasks,
      },
      WebhookHealthCheck: {
        Interval: 10080,
        ...TaskList.WebhookHealthCheck,
      },
      IntegrationSync: {
        Interval: 60,
        ...TaskList.IntegrationSync,
      },
      ArrCalendarSync: {
        Interval: 60,
        ...TaskList.ArrCalendarSync,
      },
      DownloadQueueSync: {
        Interval: 5,
  ...
  beginTask(taskEnumKey) {
    switch (taskEnumKey) {
      ...
      case "Backup":
        this.addBackupTask();
        break;
      case "JellyfinPlaybackReportingPluginSync":
      case "RefreshDashboardStats":
      case "ClearStaleTasks":
      case "WebhookHealthCheck":
  ...
  addGenericTask(taskEnumKey) {
    const task = this.taskManager.taskList[taskEnumKey];
    const success = this.taskManager.addTask({
      task,
      onComplete: async () => {
        await this.getTaskHistory();
      ...
      onError: async (error) => {
        console.error(error);
        await this.getTaskHistory();
        sendUpdate("TaskError", { type: "Error", message: `${task.name} failed` });
      },
      onExit: async () => {
        await this.getTaskHistory();
        sendUpdate("TaskError", { type: "Error", message: `${task.name} stopped` });
      },
    });
    if (success) {
      this.taskManager.startTask(task, triggertype.Automatic);
    }
  }
}
