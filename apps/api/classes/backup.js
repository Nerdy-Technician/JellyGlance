...
async function backup(refLog) {
  ...
  try {
    ...
    const directoryPath = path.join(backuppath, `backup_${now.format("YYYY-MM-DD HH-mm-ss")}.json`);
    ...
    let deleteCount = 0;
    const directoryPathDelete = backuppath;
    ...
    for (var oldBackup of fileData) {
      const oldBackupFile = path.join(backuppath, oldBackup.name);
