...
async function runPartialSyncTask(triggerType = triggertype.Automatic) {
  try {
    ...
    const newMediaCount =
      Number(syncResults?.stats?.itemsAdded || 0) +
      Number(syncResults?.stats?.episodesAdded || 0) +
      Number(syncResults?.stats?.seasonsAdded || 0);

    if (syncResults?.success && newMediaCount > 0) {
