...
const jf_activity_watchdog_mapping = (item) => ({
  ...
  NowPlayingItemId: item.NowPlayingItem?.SeriesId || item.NowPlayingItem?.Id,
  NowPlayingItemName: item.NowPlayingItem?.Name,
  EpisodeId: item.NowPlayingItem?.SeriesId ? item.NowPlayingItem.Id : null,
  SeasonId: item.NowPlayingItem?.SeasonId || null,
  SeriesName: item.NowPlayingItem?.SeriesName || null,
  UserPrimaryImageTag: item.UserPrimaryImageTag || null,
  SeasonNumber: item.NowPlayingItem?.ParentIndexNumber ?? null,
  EpisodeNumber: item.NowPlayingItem?.IndexNumber ?? null,
  PlaybackDuration: item.PlaybackDuration !== undefined ? item.PlaybackDuration : 0,
  PlayMethod: item.PlayState?.PlayMethod ?? item.PlayMethod,
  ActivityDateInserted:
