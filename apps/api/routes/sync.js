...
async function scanSingleLibrary(library) {
  let startIndex = 0;
  const increment = 200;
  const totals = {
    insertedItemsCount: 0,
    updatedItemsCount: 0,
    insertedSeasonsCount: 0,
    updatedSeasonsCount: 0,
    insertedEpisodeCount: 0,
    updatedEpisodeCount: 0,
    insertItemInfoCount: 0,
    updateItemInfoCount: 0,
    insertEpisodeInfoCount: 0,
    updateEpisodeInfoCount: 0,
  };

  const wsMessage = "Scanning Library : " + library.Name;
  sendUpdate(syncTask.wsKey, { type: "Update", message: wsMessage });
  ...
  let libraryItems = await API.getItemsFromParentId({
    ...
    syncTask,
    wsMessage,
    params: { startIndex, increment },
  });

  while (libraryItems.length !== 0) {
    const libraryItemsWithParent = libraryItems.map((item) => ({
      ...item,
      ParentId: library.Id,
    }));
    ...
    if (library_items.length > 0) {
      const counts = await syncLibraryItems(library_items);
      totals.insertedItemsCount += Number(counts.insertedItemsCount);
      totals.updatedItemsCount += Number(counts.updatedItemsCount);
    }

    if (seasons.length > 0) {
      const counts = await syncSeasons(seasons);
      totals.insertedSeasonsCount += Number(counts.insertSeasonsCount);
      totals.updatedSeasonsCount += Number(counts.updateSeasonsCount);
    }

    if (episodes.length > 0) {
      const counts = await syncEpisodes(episodes);
      totals.insertedEpisodeCount += Number(counts.insertEpisodeCount);
      totals.updatedEpisodeCount += Number(counts.updateEpisodeCount);
    }

    const infoCount = await syncItemInfo([...seasons, ...episodes], library_items);
    totals.insertItemInfoCount += Number(infoCount.insertItemInfoCount);
    totals.updateItemInfoCount += Number(infoCount.updateItemInfoCount);
    totals.insertEpisodeInfoCount += Number(infoCount.insertEpisodeInfoCount);
    totals.updateEpisodeInfoCount += Number(infoCount.updateEpisodeInfoCount);
...
router.post("/library/:libraryId", async (req, res) => {
  const uuid = randomUUID();
  try {
    const config = await new configClass().getConfig();
    if (config.error) {
    ...
    const libraries = await API.getLibraries();
    const library = libraries.find((item) => item.Id === req.params.libraryId);
    if (!library) {
    ...
    syncTask = {
      loggedData: [],
      uuid,
      wsKey: `LibraryScanTask:${library.Id}`,
      taskName: taskName.fullsync,
    };

    await logging.insertLog(uuid, triggertype.Manual, taskName.fullsync);
    syncTask.loggedData.push({ color: "yellow", Message: "Manual library scan started for " + library.Name });

    const librarySync = new sync();
    await librarySync.insertData("jf_libraries", [jf_libraries_mapping(library)], jf_libraries_columns);
    const stats = await scanSingleLibrary(library);
    await updateLibraryStatsData();
    ...
    syncTask.loggedData.push({ color: "lawngreen", Message: "Manual library scan completed for " + library.Name });
    await logging.updateLog(uuid, syncTask.loggedData, taskstate.SUCCESS);
    res.send({ success: true, library: { Id: library.Id, Name: library.Name }, stats });
  } catch (error) {
    console.error(error);
    if (syncTask?.loggedData) {
      syncTask.loggedData.push({ color: "red", Message: getErrorLineNumber(error) + ": Error: " + error });
      await logging.updateLog(uuid, syncTask.loggedData, taskstate.FAILED);
    }
    res.status(500).send({ error: "Manual library scan failed", message: error.message });
  }
});
